...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        ...
        Version = "v1",
        Description =
            "API for managing documents, users, roles and approvals."
    });
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (adminUser == null)
    {
        ...
        if (!createResult.Succeeded)
        {
            ...
            throw new InvalidOperationException(
                $"Could not create default administrator: {errors}");
        }
    ...
    {
        // ----------------------------------------------------
        // Make sure existing admin can log in with the
        // known development password.
        // ----------------------------------------------------
        ...
        if (!passwordValid)
        {
            ...
            if (!resetResult.Succeeded)
            {
                ...
                throw new InvalidOperationException(
                    $"Could not reset administrator password: {errors}");
            }
